...
- Local Blacksmith setup (this VPS): already authenticated to org `Aguanita-LLC` (verify with `blacksmith auth status`); this checkout's Blacksmith-capable fork is `origin = Aguanita-LLC/openclaw` and `upstream = openclaw/openclaw`. Preferred Testbox flow uses the repo wrappers: `blacksmith testbox warmup ci-check-testbox.yml --ref main --idle-timeout 90` -> `pnpm testbox:claim --id <tbx_id>` -> `pnpm testbox:sanity -- --id <tbx_id>` -> `pnpm testbox:run --id <tbx_id> -- "OPENCLAW_TESTBOX_REMOTE_RUN=1 pnpm check:changed"`. In-box command MUST use `OPENCLAW_TESTBOX_REMOTE_RUN=1` (already-on-box marker), NOT `OPENCLAW_TESTBOX=1`: the latter makes the in-box `check:changed` re-delegate to a fresh box and fail with `Missing Testbox id` (`OPENCLAW_TESTBOX=1 pnpm check:changed` is host-side only, where it auto-delegates). `check:changed` is typecheck+lint+guards only; for in-box vitest use `test:changed`/`pnpm test`. Stop one-off boxes when done with `blacksmith testbox stop --id <tbx_id>` (the `--id` flag is required; a positional id is rejected).
...
- **Deploy via `rebuild.sh`, never bare `pnpm build`.** To deploy code to the live host stack run the canonical `~/godwind/vps-infrastructure/rebuild.sh`. A bare `pnpm build` breaks the stack two ways every time: (1) `dist/` is owned by `opc` (container uid 1000) but the host build user is `ubuntu` (uid 1001) → `EACCES`; (2) `pnpm build` cleans `dist/` and builds the server only — the web dashboard's Control UI is a separate `pnpm ui:build`, so it gets wiped ("Control UI assets not found"). `rebuild.sh` does chown→`pnpm build`→`pnpm ui:build`→restore-`opc`→recreate. Full host-stack/deploy facts: `~/.codex/HOST-STACK.md`.
